...
color_clusters_2 <- c("#DC050C", "#FB8072", "#1965B0", "#FDB462", "#FF7F00", "#B17BA6", "#882E72")
...
pca_out <- prcomp(expr_median_sample_tbl[,-(c(1))], center = TRUE, scale. = TRUE)   # PCA analysis of samples based on median expression
...
ggdf$batch <- as.factor(sample_metadata$run_batch[mm])
...
ggplot(ggdf, aes(x = PCA1, y = PCA2, color = batch)) +         # PCA plot of samples based on marker expression levels
  geom_point(size = 8, alpha = 0.8) +
  theme_classic() +
...
nmc <- 22                                   # DEFINES THE NUMBER OF FINAL POPULATIONS TO COLLAPSE THE SOM NODES INTO
...
mc <- ConsensusClusterPlus(t(codes), maxK = nmc, reps = 100,
                           ...
                           distance = "euclidean", seed = 1234)  # takes the 100 nodes from FlowSOM and further clusters them to 22 new clusters
...
levels_clusters_merged <- c("CD4 T cells", "CD8 T cells", "B cells", "Myeloid",
...
plot_clustering_heatmap_wrapper(collated_expr = collated_expr[, lineage_markers_ord],
                                ...
                                color_clusters = color_clusters_2)
...
ggplot(codes_dr, aes(x = tSNE1, y = tSNE2, color = code_clustering1m, size = size)) +
  ...
  theme_bw() +
  scale_color_manual(values = color_clusters_2) +
...
ggplot(tsne_codes_df, aes(x = sample_ids, y = Freq, fill = code_clustering1)) +
  ...
  scale_fill_manual(values = color_clusters_2)

# NEW CODE NOT IN THE PAPER!!!
# purpose - instead of displaying select 2000 cells from each sample in a t-SNE plot and coloring them
# by the SOM node and subsequent cluster assignment, we take the t-SNE visualization of the 100 nodes
# from the SOM and plot them out from sample to sample - changes in populations should be visible in
# the size of each point. To correct for the fact that each sample has a different number of cells to
# begin with, the cell numbers are normalized for each sample to the total acquired and the RELATIVE
# frequencies are what determines the size of each point in the plots
...
cell_freq_table <- t(t(cell_count_table) / colSums(cell_count_table)) * 100
tsne_codes_df <- as.data.frame(cell_freq_table)                       # generates the core df to start visualizing the nodes within each sample separately
tsne_codes_df$X <- tsne_out$Y[,1]
tsne_codes_df$Y <- tsne_out$Y[,2]
tsne_codes_df$code_clustering1 <- factor(code_clustering1m)

ggplot(tsne_codes_df, aes(x = X, y = Y, color = code_clustering1, size = Freq)) +   # plot the t-SNE visualization per sample
  geom_point(alpha = 0.9) +
  theme_bw() +
  facet_wrap(~sample_ids) +
  scale_color_manual(values = color_clusters_2) +
  guides(color = guide_legend(override.aes = list(size = 4), ncol = 2))

# t-SNE plot separated into N/Adj pancreas versus PDAC - all samples within each condition agglomerated
# into a single tSNE plot -> the final figure will have two tSNE plots to compare between each other

mm <- match(tsne_codes_df$sample_ids,sample_metadata$sample_id)
tsne_codes_df$condition <- factor(sample_metadata$condition[mm])

tsne_codes_grouped <- group_by(tsne_codes_df,condition,cell_clustering_som)
tsne_codes_summed <- tsne_codes_grouped %>% summarize(Freq = sum(Freq))
tsne_codes_totals <- tsne_codes_summed %>% group_by(condition) %>% summarize(Freq = sum(Freq))
tsne_codes_summed$X <- tsne_codes_df[1:100,4]
tsne_codes_summed$Y <- tsne_codes_df[1:100,5]
mm <- match(tsne_codes_summed$condition,tsne_codes_totals$condition)
tsne_codes_summed$adj_Freq <- (tsne_codes_summed$Freq/tsne_codes_totals$Freq[mm])*100
mm <- match(tsne_codes_summed$cell_clustering_som,tsne_codes_df$cell_clustering_som)
tsne_codes_summed$code_clustering1 <- tsne_codes_df$code_clustering1[mm]

ggplot(tsne_codes_summed, aes(x = X, y = Y, color = code_clustering1, size = adj_Freq)) +   # plot the t-SNE visualization per sample
  geom_point(alpha = 0.9) +
  theme_bw() +
